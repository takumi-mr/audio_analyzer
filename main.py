...
def main(cli_args: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        ...
        "--input",
        required=True,
        help="解析対象の音声ファイルパス (WAV, MP3, FLAC, OGG等)",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="result.json",
        help="解析結果を出力するJSONファイルパス (デフォルト: result.json)",
    )
    parser.add_argument(
        "-s",
        "--strategies",
    ...
    parser.add_argument(
        "-r",
        "--reference",
        help="類似度計算(similarity)時に比較対象とする参照音声ファイルパス",
    )
    parser.add_argument(
        "--no-separation",
        action="store_true",
        help="DemucsによるAI音源分離フィルターをスキップします",
    )
    parser.add_argument(
        "--cutoff",
        type=float,
        default=150.0,
        help="ドラム低域抽出用フィルターのカットオフ周波数 (Hz, デフォルト: 150.0)",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=0.2,
        help="アタック音強調用コンプレッサーの閾値 (デフォルト: 0.2)",
    )
    parser.add_argument(
        "--ratio",
        type=float,
        default=3.0,
        help="アタック音強調用コンプレッサーのレシオ (デフォルト: 3.0)",
    )
    ...
    filters.append(
        CompressorFilter(
            target_key="target_drums_low", threshold=args.threshold, ratio=args.ratio
        )
    )
    ...
    if "similarity" in selected_keys:
        if not args.reference:
            print(
                "Error: similarity 戦略の実行には --reference (-r) オプションが必須です。",
                file=sys.stderr,
            )
            sys.exit(1)
