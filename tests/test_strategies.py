...
class TestAnalysisStrategies(unittest.TestCase):
    def setUp(self):
        ...
        for beat_t in [0.0, 0.5, 1.0, 1.5]:
            ...
            if idx + click_len <= len(data):
                data[idx : idx + click_len] += np.sin(
                    2 * np.pi * 1000 * np.linspace(0, 0.05, click_len, endpoint=False)
                )

        self.signal_120bpm = AudioSignal(
            data=data, sample_rate=self.sr, duration_sec=self.duration
        )
        ...
        for beat_t in [0.0, 0.4, 0.8, 1.2, 1.6]:
            ...
            if idx + click_len <= len(data_150):
                data_150[idx : idx + click_len] += np.sin(
                    2 * np.pi * 1000 * np.linspace(0, 0.05, click_len, endpoint=False)
                )

        self.signal_150bpm = AudioSignal(
            data=data_150, sample_rate=self.sr, duration_sec=self.duration
        )
    ...
    def test_simple_beat_strategy(self):
        ...
        self.assertAlmostEqual(
            result["tempo_bpm"], 120.0, delta=10.0
        )  # 簡易検出の許容誤差
    ...
    def test_sliding_window_beat_strategy(self):
        ...
        for beat_t in np.arange(0, 10.0, 0.5):
            ...
            if idx + click_len <= len(long_data):
                long_data[idx : idx + click_len] += np.sin(
                    2 * np.pi * 1000 * np.linspace(0, 0.05, click_len, endpoint=False)
                )

        long_signal = AudioSignal(
            data=long_data, sample_rate=self.sr, duration_sec=10.0
        )
    ...
    def test_audio_similarity_strategy(self):
        ...
        signals = {"target": self.signal_120bpm, "reference": self.signal_150bpm}
        result = strategy.analyze(signals)
