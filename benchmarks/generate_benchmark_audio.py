...
def synth_lead_vocal(sr: int, notes: list[tuple[str, float, float]], total_dur: float) -> np.ndarray:
...
def generate_track_acoustic_ballad(out_dir: str, mix_out_path: str):
    ...
    for b in range(total_beats):
        ...
        if half_idx + len(hihat) <= total_samples:
            drums[half_idx:half_idx+len(hihat)] += hihat * (0.6 if is_chorus else 0.3)
    ...
    for bar, (chord_name, bass_note, chord_notes) in enumerate(chord_progression):
    ...
    vocal_notes = [
        ...
        ("E4", 3.8, 0.7), ("D4", 4.6, 0.6), ("C4", 5.4, 1.0),
        # Chorus (Bar 2, 3: 6.67〜13.33s) - 高音サビフック
        ("A4", 6.8, 0.9), ("G4", 7.8, 0.8), ("E4", 8.8, 1.2),
        ("C5", 10.2, 1.0), ("A4", 11.4, 0.9), ("G4", 12.4, 0.8),
    ]
...
def generate_track_city_funk(out_dir: str, mix_out_path: str):
    ...
    for b in range(total_beats):
        ...
        for sixteenth in range(4):
            ...
            if t_six + len(hihat) <= total_samples:
                drums[t_six:t_six+len(hihat)] += hihat * (0.6 if sixteenth % 2 == 0 else 0.35)
    ...
    vocal_notes = [
        ...
        ("A4", 2.2, 0.4), ("C5", 2.8, 0.4), ("A4", 3.4, 0.5),
        # Chorus (Bar 2..4: 4.14〜10.34s)
        ("B4", 4.3, 0.6), ("D5", 5.0, 0.7),
        ("E5", 6.3, 0.8), ("G4", 7.2, 0.5), ("B4", 7.8, 0.6),
        ("A4", 8.4, 0.7), ("C5", 9.2, 0.8),
    ]
    vocals = synth_lead_vocal(SR, vocal_notes, total_dur)
