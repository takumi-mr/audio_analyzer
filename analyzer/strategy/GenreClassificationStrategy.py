...
class GenreClassificationStrategy(IAnalysisStrategy):
    ...
    def analyze(
        ...
        print("[Strategy: Genre] 音響特徴量を抽出してジャンルを特定中...")
        ...
        centroid = float(
            np.mean(
                librosa.feature.spectral_centroid(y=signal.data, sr=signal.sample_rate)
            )
        )
        # ゼロ交差率 (アタック感・激しさ)
        ...
        rolloff = float(
            np.mean(
                librosa.feature.spectral_rolloff(
                    y=signal.data, sr=signal.sample_rate, roll_percent=0.85
                )
            )
        )
        ...
        # テンポが軽快で、音質は明るいがノイズ感（flatness）が低く抑えられている場合
            ...
            genre = "Pop"
            confidence = min(
                0.92, 0.3 + (centroid / 3500.0) + (1.0 - flatness * 100) * 0.3
            )
