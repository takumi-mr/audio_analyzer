...
class SimpleBeatStrategy(IAnalysisStrategy):
    ...
    def analyze(
        ...
        if "target_drums" in signals and "target_bass" in signals:
            print(
                "[Strategy: Simple] 'target_drums' と 'target_bass' を合成してリズムを解析します (倍検出防止)"
            )
            ...
            print(
                "[Strategy: Simple] 解析対象として 'target_drums' シグナルを採用しました。"
            )
        ...
        print("[Strategy: Simple] librosaを用いてBPMを正確に検出中...")
