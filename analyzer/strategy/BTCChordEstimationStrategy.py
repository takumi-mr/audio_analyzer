...
class BTCChordEstimationStrategy(IAnalysisStrategy):
    ...
    def __init__(
        self, device: torch.device | None = None, use_gpu: bool = True
    ) -> None:
    ...
    def _init_model(self) -> None:
        try:
            ...
            print(
                f"[Warning: BTCChord] BTC モデルの初期化に失敗しました。フォールバックを使用します: {e}"
            )
            self.is_ready = False
    ...
    def analyze(
        ...
        if not self.is_ready or self.model is None:
            print(
                "[Strategy: BTCChord] BTCモデルが利用できないため、物理ルールベースHMMにフォールバックします。"
            )
            from analyzer.strategy.ChordEstimationStrategy import (
        ...
        if "target_other" in signals:
            print(
                "[Strategy: BTCChord] 音源分離 'target_other' を伴奏特徴抽出に使用します。"
            )
        ...
        print(
            f"[Strategy: BTCChord] SOTA BTC Transformer による深層学習コード進行推定中 (デバイス: {self.device})..."
        )
        ...
        beat_times = librosa.frames_to_time(
            beat_frames, sr=beat_sr, hop_length=hop_length_beat
        )
        ...
        while curr_sample < len(y_22k):
            ...
            if tmp.shape[1] > n_timestep:
                ...
                tmp = np.pad(
                    tmp, ((0, 0), (0, n_timestep - tmp.shape[1])), mode="constant"
                )
        ...
        with torch.no_grad():
            feat_t = (
                torch.tensor(norm_cqt, dtype=torch.float32).unsqueeze(0).to(self.device)
            )
        ...
        if has_bass:
            ...
            try:
                ...
                for b_idx in range(n_beats):
                    ...
                    t_en = (
                        beat_times[b_idx + 1] if b_idx + 1 < n_beats else total_duration
                    )
        ...
        for b_idx in range(n_beats):
            ...
            if np.any(mask):
                ...
                # 最も近いフレームを採用
                nearest_idx = min(round(t_st / frame_sec), total_frames - 1)
            ...
            chord_meta = self.vocab.get(
                best_idx, {"chord": "N", "root": None, "suffix": "", "triad": "none"}
            )
            raw_chord = chord_meta["chord"]
            root_name = chord_meta.get("root")
            root_pitch = chord_meta.get("root_pitch")
            _suffix = chord_meta.get("suffix", "")

            # ベース音との照合によるオンコード（分数コード）合成
            bass_p = (
                beat_bass_pitches[b_idx] if b_idx < len(beat_bass_pitches) else None
            )

            if (
                raw_chord not in ["N", "X"]
                and root_pitch is not None
                and bass_p is not None
                and bass_p != root_pitch
            ):
        ...
        if all_n and len(signals) > 0:
            print(
                "[Warning: BTCChord] BTC出力が全てNとなったため、物理ルールベースHMMにフォールバックします。"
            )
            from analyzer.strategy.ChordEstimationStrategy import (
                ChordEstimationStrategy,
            )
