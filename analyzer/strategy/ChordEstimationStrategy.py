    ...
    def __init__(self, engine: str = "hybrid") -> None:
        ...
        if self.engine in ["hybrid", "btc"]:
            try:
                ...
                print(
                    f"[ChordEstimationStrategy] BTC engine unavailable, fallback to heuristic: {e}"
                )
                self.btc_strategy = None

        self.pitch_classes: list[str] = [
            "C",
            "C#",
            "D",
            "D#",
        ...
        self.extra_pitch_map: dict[
            int, list[tuple[int, list[int]]]
        ] = {}  # chord_idx -> list of (extra_pitch, triad_pitches)

        chord_idx = 0
        for _mode_name, intervals, suffix, prior in chord_types:
            for i in range(12):
                template = np.zeros(12)
                for idx, interval in enumerate(intervals):
                    if idx == 0:
                        weight = 1.4  # ルート音の結合度を高く設定
                    elif idx == 1:
                        weight = 1.0
                    elif idx == 2:
                        weight = 0.8
                ...
                triad_pitches = [
                    (i + intervals[k]) % 12 for k in range(min(3, len(intervals)))
                ]
                extras = []
                if len(intervals) >= 4:
                    p7 = (i + intervals[3]) % 12
        ...
        self.templates_t = torch.tensor(
            self.templates, dtype=torch.float32, device=self.device
        )  # (144, 12)
        self.priors_t = torch.tensor(
            self.chord_priors, dtype=torch.float32, device=self.device
        )  # (144, 1)
        self.chord_roots_t = torch.tensor(
            self.chord_roots_arr, dtype=torch.long, device=self.device
        )  # (144,)
    ...
    def analyze(
        ...
        if engine_to_use == "btc":
            if self.btc_strategy is not None and self.btc_strategy.is_ready:
                ...
                print(
                    "[Strategy: Chord] BTC Strategy 利用不可のため、ルールベース物理HMMで解析します。"
                )
        ...
        if "target_other" in signals:
            print(
                "[Strategy: Chord] 音源分離の 'target_other' を伴奏特徴抽出に使用します。"
            )
        ...
        print(
            f"[Strategy: Chord] 高精度PyTorch-Neuralコード進行推定中 (デバイス: {self.device})..."
        )
        ...
        chroma_cqt = librosa.feature.chroma_cqt(
            y=signal.data, sr=sr, hop_length=hop_length, tuning=tuning
        )
        ...
        chroma_t = torch.tensor(
            chroma_sync, dtype=torch.float32, device=self.device
        )  # (12, n_beats)
        ...
        for i in range(12):
            ...
            val = (
                chroma_t[i, :]
                - 0.30 * chroma_t[sub_fifth, :]
                - 0.15 * chroma_t[sub_third, :]
            )
        ...
        if has_bass:
            ...
            try:
                sr_down = 4000
                y_bass_down = librosa.resample(
                    y_bass, orig_sr=sr_bass, target_sr=sr_down
                )
                hop_yin = 64
                f0_bass = librosa.yin(
                    y_bass_down,
                ...
                yin_times = librosa.frames_to_time(
                    np.arange(len(f0_bass)), sr=sr_down, hop_length=hop_yin
                )
            ...
            chroma_b_cqt = librosa.feature.chroma_cqt(
                y=y_bass, sr=sr, hop_length=hop_length, tuning=bass_tuning
            )
            chroma_b_cens = librosa.feature.chroma_cens(
                y=y_bass, sr=sr, hop_length=hop_length, tuning=bass_tuning
            )
            chroma_bass_raw = 0.5 * chroma_b_cqt + 0.5 * chroma_b_cens

            chroma_bass_sync = librosa.util.sync(
                chroma_bass_raw, beat_frames.tolist(), aggregate=np.median
            )
            bass_t = torch.tensor(
                chroma_bass_sync, dtype=torch.float32, device=self.device
            )  # (12, n_beats)
        ...
        if engine_to_use == "hybrid" and self.btc_model is not None:
            try:
                ...
                y_22k = (
                    librosa.resample(signal.data, orig_sr=sr, target_sr=target_hz)
                    if sr != target_hz
                    else signal.data
                )
                ...
                while curr_s < len(y_22k):
                    ...
                    if len(seg) < chunk_samples:
                        seg = np.pad(
                            seg, (0, chunk_samples - len(seg)), mode="constant"
                        )
                    tmp = librosa.cqt(
                        seg,
                        sr=target_hz,
                ...
                with torch.no_grad():
                    f_t = (
                        torch.tensor(norm_cqt, dtype=torch.float32)
                        .unsqueeze(0)
                        .to(self.device)
                    )
                    for t in range(num_inst):
                        out_l = self.btc_model(
                            f_t[:, n_timestep * t : n_timestep * (t + 1), :]
                        )
                        btc_logits_list.append(out_l.cpu())

                btc_logits = torch.cat(btc_logits_list, dim=1).squeeze(
                    0
                )  # (total_frames, 170)
                # N と X の学習時不均衡バイアスを適正化
                btc_logits[:, 169] -= 6.0
                btc_logits[:, 168] -= 6.0
                btc_probs = torch.softmax(
                    btc_logits, dim=-1
                ).numpy()  # (total_frames, 170)
                ...
                beat_btc_t = torch.tensor(
                    beat_btc_p, dtype=torch.float32, device=self.device
                )  # (170, n_beats)
                # 144 クラス空間への射影
                ...
                print(
                    "  - [Hybrid Fusion] SOTA BTC Transformer テンソル射影融合完了 (重み: 2.5)"
                )
            except Exception as e:
        ...
        for chord_c, extras in self.extra_pitch_map.items():
            for p_extra, triad_pitches in extras:
                ...
                absence = torch.clamp(
                    (thresh_adaptive - ratio) / thresh_adaptive, min=0.0, max=1.0
                )
        ...
        key_name = self.pitch_classes[best_key] + (
            " Major" if best_mode == "major" else " Minor"
        )
        print(
            f"  - 曲全体の自動キー推定: {key_name} (機能和声Priorおよびダイアトニック優遇を適用します)"
        )
        ...
        functional_priors = torch.ones(
            n_states, dtype=torch.float32, device=self.device
        )
        ...
        for idx in range(n_states):
            ...
            if best_mode == "major":
                ...
                if (
                    deg == 0
                ):  # I度 (Tonic): Major / M7 は高確率、7(Dominant)はトニックでは不協和のため抑制
                    ...
                    deg == 7
                ):  # V度 (Dominant): 7th / 7(9) / Major は自然、M7は原則不出現
                    if suffix in ["7", "7(9)", ""]:
                ...
                if deg == 0:  # i度 (Tonic): minor / m7 は高確率、Majorや7thは抑制
                    ...
                    deg == 7
                ):  # v度 (Dominant): 7th (和声短音階強進行) または m/m7 (自然短音階)
                    if suffix in ["7", "7(9)", "m", "m7"]:
                        functional_priors[idx] = 1.25
                elif deg == 3:  # bIII度 (平行長調Tonic): Major / M7 は自然
                    if suffix in ["", "M7", "M7(9)"]:
                        functional_priors[idx] = 1.25
                elif (deg == 5 and suffix in ["m", "m7"]) or (
                    deg in (8, 10) and suffix in ["", "M7", "7"]
                ):
        ...
        deg_map = (
            diatonic_degrees_major if best_mode == "major" else diatonic_degrees_minor
        )
        ...
        for idx_b, path_idx in enumerate(path):
            ...
            bass_p = (
                beat_bass_pitches[idx_b] if idx_b < len(beat_bass_pitches) else None
            )
        ...
        if len(beat_chord_info) >= 3:
            for i in range(1, len(beat_chord_info) - 1):
                if (
                    beat_chord_info[i - 1]["chord"] == beat_chord_info[i + 1]["chord"]
                    and beat_chord_info[i]["chord"] != beat_chord_info[i - 1]["chord"]
                ):
        ...
        if len(beat_chord_info) > 0:
            ...
            if (
                len(chords_sequence) > 1
                and (chords_sequence[0]["end_sec"] - chords_sequence[0]["start_sec"])
                < 0.35
            ):
        ...
        return {
            "status": "success",
            "tempo": round(
                float(tempo[0]) if isinstance(tempo, np.ndarray) else float(tempo), 1
            ),
