...
class AudioSimilarityStrategy(IAnalysisStrategy):
    ...
    def analyze(
        ...
        if not target or not reference:
            raise ValueError(
                "類似度計算には 'target' と 'reference' の両方の音声信号が必要です。"
            )

        print(
            "[Strategy: Similarity] 音階 (Chroma) と音色 (MFCC) に基づくハイブリッド類似度を計算中..."
        )

        # 1. 音色類似度 (MFCC コサイン類似度) の計算
        target_mfcc = np.mean(
            librosa.feature.mfcc(y=target.data, sr=target.sample_rate, n_mfcc=13),
            axis=1,
        )
        ref_mfcc = np.mean(
            librosa.feature.mfcc(y=reference.data, sr=reference.sample_rate, n_mfcc=13),
            axis=1,
        )

        denom_mfcc = np.linalg.norm(target_mfcc) * np.linalg.norm(ref_mfcc)
        timbre_similarity = (
            float(np.dot(target_mfcc, ref_mfcc) / denom_mfcc) if denom_mfcc > 0 else 0.0
        )
        # [-1, 1] -> [0, 1]
        timbre_score = (timbre_similarity + 1.0) / 2.0

        # 2. 音階/コード進行類似度 (Chroma CENS コサイン類似度) の計算
        target_chroma = np.mean(
            librosa.feature.chroma_cens(y=target.data, sr=target.sample_rate), axis=1
        )
        ref_chroma = np.mean(
            librosa.feature.chroma_cens(y=reference.data, sr=reference.sample_rate),
            axis=1,
        )

        denom_chroma = np.linalg.norm(target_chroma) * np.linalg.norm(ref_chroma)
        pitch_similarity = (
            float(np.dot(target_chroma, ref_chroma) / denom_chroma)
            if denom_chroma > 0
            else 0.0
        )
        ...
        weights = (
            params.get("weights", {"timbre": 0.5, "pitch": 0.5})
            if params
            else {"timbre": 0.5, "pitch": 0.5}
        )
