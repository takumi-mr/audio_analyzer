...
class KeyDetectionStrategy(IAnalysisStrategy):
    ...
    def analyze(self, signals: Dict[str, AudioSignal], params: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if "target_other" in signals and "target_vocal" in signals:
            print("[Strategy: Key] 'target_other' と 'target_vocal' を合成して和声と主旋律からキーを推定します。")
            y = signals["target_other"].data + signals["target_vocal"].data
            sr = signals["target_other"].sample_rate
        elif "target_other" in signals:
            print("[Strategy: Key] 解析対象として 'target_other' シグナルを採用しました。")
            ...
            print("[Strategy: Key] 解析対象として 'target_harmonic' シグナルを採用しました。")
            y = signals["target_harmonic"].data
            sr = signals["target_harmonic"].sample_rate
        elif "target_vocal" in signals:
            print("[Strategy: Key] 解析対象として 'target_vocal' シグナルを採用しました。")
        ...
        print("[Strategy: Key] チューニング補正とハイブリッドクロマ(CQT+CENS)により主キーを推定中...")
