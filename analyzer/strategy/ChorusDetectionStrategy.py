    ...
    def analyze(
        ...
        for key in ["target_vocal", "target_harmonic", "target"]:
            if key in signals:
                signal = signals[key]
                print(
                    f"[Strategy: Chorus] 解析対象として '{key}' シグナルを採用しました。"
                )
                break
        ...
        print(
            "[Strategy: Chorus] 音響特徴量および構造分析によりサビ(Chorus)区間を検出中..."
        )
        ...
        norm_energy = (sec_energy - np.min(sec_energy)) / (
            denom_energy if denom_energy > 0 else 1.0
        )

        denom_centroid = np.max(sec_centroid) - np.min(sec_centroid)
        norm_centroid = (sec_centroid - np.min(sec_centroid)) / (
            denom_centroid if denom_centroid > 0 else 1.0
        )
        ...
        if ref_chroma_norm > 0:
            ...
            for s in range(n_seconds - span_len):
                if s + span_len <= best_start or s >= best_end:
                    ...
                    if test_norm > 0:
                        sim = np.dot(ref_chroma, test_chroma) / (
                            ref_chroma_norm * test_norm
                        )
                        if (
                            sim > 0.92
                            and np.mean(sec_energy[s : s + span_len])
                            > np.mean(sec_energy) * 0.8
                        ):
                            chorus_sections.append(
                                {"start_sec": float(s), "end_sec": float(s + span_len)}
                            )
                            break
