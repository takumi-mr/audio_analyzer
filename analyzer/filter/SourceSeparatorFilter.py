...
class SourceSeparatorFilter(IAudioFilter):
    ...
    def apply(self, signals: dict[str, AudioSignal]) -> dict[str, AudioSignal]:
        ...
        print(
            f"[Filter: SourceSeparator] Demucs ({self.model_name}) を用いて '{self.target_key}' の音源分離を開始します..."
        )
        ...
        if self._separator is None:
            print(
                f"[Filter: SourceSeparator] モデル '{self.model_name}' をロード中 (デバイス: {device})..."
            )
        ...
        _, separated = self._separator.separate_tensor(
            wav_tensor, sr=signal.sample_rate
        )
            ...
            if stem_name in separated:
                ...
                if demucs_sr != target_sr:
                    stem_resampled = librosa.resample(
                        stem_mono, orig_sr=demucs_sr, target_sr=target_sr
                    )
                else:
                ...
                print(
                    f"[Filter: SourceSeparator] 分離完了: '{output_key}' (長さ: {duration:.2f}秒)"
                )
