...
class CompressorFilter(IAudioFilter):
    ...
    def __init__(self, target_key: str = "target", threshold: float = 0.3, ratio: float = 4.0, gain: float = 1.2):
    ...
    def apply(self, signals: Dict[str, AudioSignal]) -> Dict[str, AudioSignal]:
        ...
        print(f"[Filter: Compressor] '{self.target_key}' に対しコンプレッションを適用中 (threshold={self.threshold}, ratio={self.ratio})...")
        ...
        if np.any(over_threshold):
            ...
            compressed_vals = self.threshold + (abs_data[over_threshold] - self.threshold) / self.ratio
