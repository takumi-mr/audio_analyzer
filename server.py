...
def read_index():
    ...
    if not os.path.exists(index_path):
        raise HTTPException(
            status_code=404,
            detail="Frontend HTML file not found in 'templates/' directory.",
        )
...
async def analyze_audio(
    file: UploadFile = File(...), chord_engine: str | None = "hybrid"
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save temporary upload: {e}"
        ) from e
    ...
    try:
        ...
        analyzer = AudioAnalyzer(
            reader, mem_writer, SimpleBeatStrategy(), filters=filters
        )
        ...
        if os.path.exists(temp_path):
            try:
                ...
                pass
        raise HTTPException(
            status_code=500, detail=f"Analysis pipeline error: {e!s}"
        ) from e
